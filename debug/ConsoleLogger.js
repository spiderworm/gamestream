...
ConsoleLogger.prototype._log = function(receivedTime, data) {
	...
	if (this.logFull) {
		console.log(jsonUtil.stringifyForLogs(data));
	} else {
		if (this.logTimestamp) {
			console.log('time:   ', data.time);
		}
		if (this.logSpeed && data.speed !== undefined) {
			console.log('speed:  ', jsonUtil.stringifyForLogs(data.speed));
		}
		if (this.logRewrites && data.rewrite) {
			console.log('rewrite:', !!data.rewrite);
		}
		if (this.logUpdates && data.update) {
			console.log('update: ', jsonUtil.stringifyForLogs(data.update));
		}
		if (this.logReverseUpdates && data.reverseUpdate) {
			console.log('reverse update:', jsonUtil.stringifyForLogs(data.reverseUpdate));
		}
		if (this.logValues && data.values) {
			console.log('values: ', jsonUtil.stringifyForLogs(data.values));
		}
	}
	console.log('');
}
